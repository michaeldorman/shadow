...
First, we will create a grid covering the examnined area with a spatial resolution of 2 meters - 
...
The following code section plots the resulting grid (Figure \@ref(fig:heightgrid)).
...
The calculation of shade height at `r nrow(grid)` grid points took `r round(as.numeric(end_parallel-start_parallel, units="secs"), 1)` seconds on a laptop with an Intel® Core™ i5-6200U CPU @ 2.30GHz × 4 processor using the above 3-core parallel process. The resulting grid can be converted to a `RasterLayer` object of shade heights and plotted with following code section (Figure \@ref(fig:heightresult)). Note the partial shade on the roof of the 19.07-m building which is caused by the slightly taller 22.73-m building.
